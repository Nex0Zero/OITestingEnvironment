 ...
 * 
 * Link:
 * http://rosettacode.org/wiki/Hough_transform#Java
 * 
 * Info:
 * Implement the Hough transform, which is used as part of feature extraction with digital images. 
 * It is a tool that makes it far easier to identify straight lines in the source image, 
 * whatever their orientation.
 * 
 * Invoking:
 * java HoughTransform pentagon.png JavaHoughTransform.png 640 480 100
 * 		pentagon.png - Image to process
 * 		JavaHoughTransform.png - processed Image
 * 		640 - processed Image's x (width)
 * 		480 - processed Image's y (height)
 * 		100 - minimum Contrast
 * 
 */
public class HoughTransform {

	public static ArrayData houghTransform(ArrayData inputData, int thetaAxisSize, 
			int rAxisSize, int minContrast) {
	...
	public static ArrayData getArrayDataFromImage(String filename) throws IOException {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				rgbValue = (int) (((rgbValue & 0xFF0000) >>> 16) * 0.30 + ((rgbValue & 0xFF00) >>> 8) * 0.59
						+ (rgbValue & 0xFF) * 0.11);
	...
	public static void main(String[] args) throws IOException {
		...
		int minContrast = Integer.parseInt(args[4]);
		System.out.println("minimum contrast: " + minContrast);
		ArrayData outputData = houghTransform(inputData, Integer.parseInt(args[2]), Integer.parseInt(args[3]),
				minContrast);
