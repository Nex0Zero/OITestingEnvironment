...
public class Matrix
{
	...
	// Construct a nrow x ncol matrix of complex variables all equal to the
	...
	// Construct matrix with a reference to an existing nrow x ncol 2-D array of
	// complex variables
	...
	// Construct matrix with a reference to the 2D matrix and permutation index
	// of an existing ComplexMatrix bb.
	...
	// ADDITION
	// Add this matrix to matrix B. This matrix remains unaltered [non-static
	// method]
	public Matrix plus(Matrix bmat)
	{
		if ((this.nrow != bmat.nrow) || (this.ncol != bmat.ncol)) { throw new IllegalArgumentException("Array dimensions do not agree"); }
		int nr = bmat.nrow;
		int nc = bmat.ncol;
	...
	{
		if ((amat.nrow != bmat.nrow) || (amat.ncol != bmat.ncol)) { throw new IllegalArgumentException("Array dimensions do not agree"); }
		int nr = amat.nrow;
		int nc = amat.ncol;
		Matrix cmat = new Matrix(nr, nc);
		double[][] carray = cmat.getArrayPointer();
	...
	public Matrix luDecomp()
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			if (big == 0.0D)
			{
				// System.out.println("Attempted LU Decomposition of a singular
				// matrix in Matrix.luDecomp()");
				// System.out.println("NaN matrix returned and matrixCheck set
				// to false");
